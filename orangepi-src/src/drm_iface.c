...
struct sharp_memory_panel
{
	...
	struct gpio_desc *gpio_sidebutton;
	int sidebutton_irq;
};
...
static irqreturn_t sidebutton_irq_handler(int irq, void *data)
{
	struct sharp_memory_panel *panel = data;
	int value = gpiod_get_value_cansleep(panel->gpio_sidebutton);
	if (value == 0)
	{
		printk(KERN_INFO "sharp_memory: side button equal to 0\n");
		backlit_on = !backlit_on;
		g_param_backlit = backlit_on ? 1 : 0;
		gpiod_set_value(panel->gpio_backlit, g_param_backlit);
	}
...
static void power_off(struct sharp_memory_panel *panel)
{
	...
	{
		printk(KERN_INFO "---- powering off  clear screen\n");
...
int drm_probe(struct spi_device *spi)
{
	...
	panel->gpio_sidebutton = devm_gpiod_get(dev, "sidebutton", GPIOD_IN);
